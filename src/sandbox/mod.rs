...
fn create_or_update_placeholder(
	...
	let (common, expected_kind, mode_perms): (&CommonPlaceholderData, libc::mode_t, Option<libc::mode_t>) =
		match placeholder_data {
			ManagedPlaceholder::PlaceholderDir(d) => {
				(&d.common, libc::S_IFDIR, Some((d.mode & 0o7777) as libc::mode_t))
			}
			ManagedPlaceholder::PlaceholderFile(f) => {
				(&f.common, libc::S_IFREG, Some((f.mode & 0o7777) as libc::mode_t))
			}
	...
	loop {
		...
		if create_err.kind() != io::ErrorKind::AlreadyExists {
			return Err(match placeholder_data {
				...
				ManagedPlaceholder::PlaceholderSymlink(s) => BindMountSandboxError::Symlinkat(
					name.to_owned(),
					s.target.clone(),
					create_err,
				),
